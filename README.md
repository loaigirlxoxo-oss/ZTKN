...
| **Codex CLI** | **TUI で `/hooks` を打ち、ZTKN のフックを承認する**。その後 Codex を再起動 |
| Codex アプリ | 不要（フックを使わないため。下記の制約あり） |
...
| Claude Code | ○ | ○ |
| Codex CLI (`codex`) | ○ | ○ |
| **Codex アプリ（デスクトップ版）** | **○** | **× 表示されません** |

**Codex アプリはフックを実行しません。** アプリは CLI と違い `app-server` モードで動作し、
`config.toml` のフックが呼ばれません（プロセス起動ログにフック実行の記録が残らないことを実測で確認）。

そのためアプリについては、アプリ自身が書くセッション記録
（`~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl`）を読んで「実行中」だけを検出しています。
記録内の `task_started` が `task_complete` を上回っていれば実行中と判定します。

**承認待ちはアプリでは検出できません。** 承認要求はアプリと app-server の間の通信内で完結し、
ディスクに一切残らないためです（app-server の制御ソケットも外部に公開されていません）。
承認待ちも必要な場合は CLI 版の `codex` をお使いください。

なおこの方式は Codex の内部ファイル形式に依存するため、Codex の更新で動かなくなる可能性があります。

#### 既知の注意点（Codex）

- 未承認のフックは**警告なく黙ってスキップ**されます（起動時に警告は出ますが、実行時は何も出ません）
- Codex の更新で信頼レコードの形式が変わると、**既存のフックが黙って止まります**
  （`enabled` フィールドの追加で実際に発生しました）。その場合は `/hooks` で承認し直してください
