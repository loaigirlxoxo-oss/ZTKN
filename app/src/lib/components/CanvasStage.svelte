<script lang="ts">
  ...
  function valueFor(item: PanelItem): number {
    ...
    if (item.sensorSum && item.sensorSum.length) {
      ...
      for (const id of item.sensorSum) {
        const v = editor.valueOf(id);
        if (v !== undefined && Number.isFinite(v)) { sum += v; any = true; }
      }
      return any ? sum * scale : NaN;
    }
    return item.sensorSrc ? (editor.valueOf(item.sensorSrc) ?? NaN) * scale : NaN;
  }
  ...
  function buildNode(item: PanelItem): Konva.Group {
    ...
    if (item.kind === "Box") {
      ...
      const apply = () => {
        const hist = scaled(item.sensorSrc ? editor.historyOf(item.sensorSrc) : []);
        const hist2 = scaled(isDual && item.sensorSrc2 ? editor.historyOf(item.sensorSrc2) : []);
