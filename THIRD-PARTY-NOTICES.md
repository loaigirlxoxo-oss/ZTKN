...
`app/Assets/` の素材とアプリアイコンは、第三者製ソフトウェアではなく **ZTKN 側で用意したコンテンツ**です。
配布インストーラに同梱され、リポジトリにも含まれます（配布物の中身をソースから検証できるようにするため）。
ライセンスは [LICENSE](LICENSE) に従います（All rights reserved。再配布には事前許可が必要）。

利用者が自分で `app/Assets/` に追加した素材、および `app/image/` に置いた画像は上記の対象外で、
再配布可否は各素材の出所・生成物の利用規約に従います。
